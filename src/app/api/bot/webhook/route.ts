...
export async function POST(req: Request) {
  try {
    const { tenant_id, no_wa, pesan, message_type, is_new_conversation } = await req.json();
    ...
    const aiResult = await processAIResponse(pesan, no_wa, studentData || null, tenant_id, message_type || "", is_new_conversation || "true");
