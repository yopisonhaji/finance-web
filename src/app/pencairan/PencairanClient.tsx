  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6 px-1 sm:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        <Card className="col-span-1 md:col-span-1 border-blue-200 dark:border-blue-900 shadow-xl bg-gradient-to-br from-blue-500 to-blue-700 text-white overflow-hidden relative">
          ...
          <CardContent className="relative z-10">
            <div className="text-3xl sm:text-4xl font-bold tracking-tight mb-1 sm:mb-2">
        ...
        <Card className="col-span-1 md:col-span-2 border-slate-200/60 dark:border-slate-800/60 shadow-xl bg-white dark:bg-[#0f172a]">
          ...
          <CardContent>
            {bankInfo.bank && bankInfo.noRek ? (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 bg-slate-50/50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Bank Tujuan</p>
                  <p className="font-bold text-sm sm:text-base text-slate-900 dark:text-white uppercase">{bankInfo.bank}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">No. Rekening</p>
                  <p className="font-bold text-sm sm:text-base text-slate-900 dark:text-white">{bankInfo.noRek}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Atas Nama</p>
                  <p className="font-bold text-sm sm:text-base text-slate-900 dark:text-white truncate">{bankInfo.atasNama}</p>
                </div>
              </div>
            ) : (
              <div className="bg-orange-50 dark:bg-orange-900/20 text-orange-800 dark:text-orange-300 p-4 rounded-xl border border-orange-200 dark:border-orange-900/50 flex items-start gap-3">
                <AlertCircle className="w-5 h-5 mt-0.5 shrink-0" />
                <div>
                  <h4 className="font-bold text-sm">Rekening Belum Diatur</h4>
                  <p className="text-xs mt-1 text-orange-700 dark:text-orange-400">Silakan atur rekening pencairan Anda di menu <b>Pengaturan &gt; Keuangan</b> terlebih dahulu agar dapat melakukan penarikan dana.</p>
                </div>
              </div>
            )}
          </CardContent>
      ...
      <Tabs defaultValue="tarik" className="w-full flex-col">
        <TabsList className="flex w-full overflow-x-auto h-12 bg-slate-100/80 dark:bg-slate-900/80 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800">
          <TabsTrigger value="tarik" className="flex-1 rounded-lg font-bold text-sm h-full data-[state=active]:bg-white dark:data-[state=active]:bg-[#0f172a] data-[state=active]:text-blue-600 dark:data-[state=active]:text-blue-400 data-[state=active]:shadow-md transition-all whitespace-nowrap">
            Tarik Dana
          </TabsTrigger>
          <TabsTrigger value="masuk" className="flex-1 rounded-lg font-bold text-sm h-full data-[state=active]:bg-white dark:data-[state=active]:bg-[#0f172a] data-[state=active]:text-emerald-600 dark:data-[state=active]:text-emerald-400 data-[state=active]:shadow-md transition-all whitespace-nowrap">
            Uang Masuk
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="tarik" className="mt-6 space-y-6">
          <Card className="border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-[#0f172a]">
            ...
            <CardContent>
              <form onSubmit={handleTarik} className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4 max-w-lg">
                <div className="space-y-1.5 sm:space-y-2 flex-1 w-full">
                  ...
                  <Input 
                    ...
                    className="h-10 sm:h-12 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-base font-medium"
                  />
                </div>
                <Button 
                  type="submit" 
                  disabled={loading || saldo < 50000 || !bankInfo.noRek}
                  className="px-6 sm:px-8 h-10 sm:h-12 w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all"
                >
                  ...
                  <table className="w-full text-sm text-left min-w-[500px]">
                    <thead className="bg-slate-50 dark:bg-slate-900/50 text-slate-700 dark:text-slate-300">
                      <tr>
                        ...
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3 font-semibold text-xs sm:text-sm whitespace-nowrap">Nominal</th>
                        <th className="hidden sm:table-cell px-3 sm:px-4 py-2.5 sm:py-3 font-semibold text-xs sm:text-sm whitespace-nowrap">Tujuan</th>
                        <th className="px-3 sm:px-4 py-2.5 sm:py-3 font-semibold text-right text-xs sm:text-sm whitespace-nowrap">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-[#0f172a]">
                      {riwayatTarik.length === 0 ? (
                        ...
                        <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                          <td className="px-3 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm whitespace-nowrap">{formatDateTime(r.createdAt)}
                            <div className="sm:hidden mt-1 text-[10px] text-slate-500">{r.bank} - {r.noRekening}</div>
                          </td>
                          <td className="px-3 sm:px-4 py-3 sm:py-4 font-bold text-slate-900 dark:text-white text-xs sm:text-sm whitespace-nowrap">{formatter.format(r.jumlah)}</td>
                          <td className="hidden sm:table-cell px-3 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm">
                            <div className="flex flex-col">
                              <span className="font-semibold text-slate-800 dark:text-slate-200">{r.bank} - {r.noRekening}</span>
                              <span className="text-[10px] sm:text-xs text-slate-500">{r.atasNama}</span>
                            </div>
                          </td>
                          <td className="px-3 sm:px-4 py-3 sm:py-4 text-right whitespace-nowrap">
        ...
        <TabsContent value="masuk" className="mt-6">
          <Card className="border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-[#0f172a]">
            ...
            <CardContent>
              <div className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-x-auto w-full">
