    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="col-span-1 md:col-span-1 border-blue-200 dark:border-blue-900 shadow-xl bg-gradient-to-br from-blue-500 to-blue-700 text-white">
          <CardHeader className="pb-2">
            <CardTitle className="text-blue-100 flex items-center gap-2 text-lg">
              <Wallet className="w-5 h-5" /> Saldo Virtual Anda
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="col-span-1 md:col-span-2 border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-[#0f172a]">
          <CardHeader className="pb-2">
            <CardTitle className="text-slate-900 dark:text-white text-lg">Informasi Rekening Tujuan</CardTitle>
          </CardHeader>
          <CardContent>
            {bankInfo.bank && bankInfo.noRek ? (
              <div className="grid grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mb-1">Bank</p>
                  <p className="font-bold text-slate-900 dark:text-white">{bankInfo.bank}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mb-1">No. Rekening</p>
                  <p className="font-bold text-slate-900 dark:text-white">{bankInfo.noRek}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mb-1">Atas Nama</p>
                  <p className="font-bold text-slate-900 dark:text-white">{bankInfo.atasNama}</p>
                </div>
              </div>
            ) : (
      ...
      <Tabs defaultValue="tarik" className="w-full">
        <TabsList className="grid w-full grid-cols-2 h-12 bg-slate-100 dark:bg-slate-900 p-1 rounded-xl">
          <TabsTrigger value="tarik" className="rounded-lg font-bold data-[state=active]:bg-white dark:data-[state=active]:bg-[#0f172a] data-[state=active]:text-blue-600 dark:data-[state=active]:text-blue-400 data-[state=active]:shadow-sm transition-all">
            Riwayat & Tarik Dana
          </TabsTrigger>
          <TabsTrigger value="masuk" className="rounded-lg font-bold data-[state=active]:bg-white dark:data-[state=active]:bg-[#0f172a] data-[state=active]:text-emerald-600 dark:data-[state=active]:text-emerald-400 data-[state=active]:shadow-sm transition-all">
            Riwayat Uang Masuk
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="tarik" className="mt-6 space-y-6">
          <Card className="border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-[#0f172a]">
            ...
            <CardContent>
              <form onSubmit={handleTarik} className="flex items-end gap-4 max-w-lg">
                ...
                <Button 
                  ...
                  className="h-11 bg-blue-600 hover:bg-blue-700 text-white font-bold px-8"
                >
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-[#0f172a]">
                    {riwayatTarik.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="px-4 py-8 text-center text-slate-500">Belum ada riwayat penarikan dana.</td>
                      </tr>
                      ...
                      <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                        ...
                        <td className="px-4 py-3 font-bold text-slate-900 dark:text-white">{formatter.format(r.jumlah)}</td>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 dark:bg-slate-900/50 text-slate-700 dark:text-slate-300">
                    <tr>
                      ...
                      <th className="px-4 py-3 font-semibold text-right">Nominal Masuk (Bersih)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 dark:divide-slate-800 bg-white dark:bg-[#0f172a]">
                    {riwayatMasuk.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-4 py-8 text-center text-slate-500">Belum ada uang masuk.</td>
                      </tr>
                    ) : riwayatMasuk.map((r: any) => (
                      <tr key={r.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/20 transition-colors">
                        <td className="px-4 py-3">{formatDateTime(r.createdAt)}</td>
                        <td className="px-4 py-3">
                          <span className="font-mono text-xs bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">TRX-{r.id}</span>
                        </td>
